...
class Building():
    ...
    def __init__(self):
        ...
        self.number_person = 100  # number of occupants per floor
        ...
        self.space_heating_type = 0


        # Defined by the KB_General
        ...
        if self.building_type == 'residential':
            ...
            self.vol_dhw_set = 0.03 * self.number_person  # daily dwelling DHW consumption per floor [m3]
            self.renewal_air = 0  # renewal fresh air [m3/s]

        elif self.building_type == 'hotel':
            self.number_person = 2 * self.number_rooms  # number of rooms per floor
            self.vol_dhw_set = 0.03 * self.number_person  # daily dwelling DHW consumption [m3]
            self.Q_gain = 5 * self.area_floor  # occupancy and appliances heat gains [W]
            self.renewal_air = 0  # renewal fresh air [m3/s]

        else:
            self.number_person = round(self.area_floor / 9)  # number of occupants per floor (9m2 per occupant)
            self.vol_dhw_set = 0
            self.Q_gain = self.number_person * 108 + (15 + 12) * self.area_floor  # occupancy and appliances heat gains [W]
            self.renewal_air = 10 * 10 ** (-3)  # [m3/s] per person
