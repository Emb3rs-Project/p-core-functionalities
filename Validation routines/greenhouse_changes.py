...
from module.Sink.characterization.Building.Auxiliary.building_climate_api import building_climate_api
from module.Sink.characterization.Building.Auxiliary.wall_area import wall_area
from module.General.Auxiliary_General.schedule_hour import schedule_hour
from module.General.Auxiliary_General.month_last_hour import month_last_hour
from module.Sink.characterization.Building.Auxiliary.h_convection_horizontal import h_convection_horizontal
from module.Sink.characterization.Building.Auxiliary.h_convection_vertical import h_convection_vertical
from module.Sink.characterization.Building.Auxiliary.ht_radiation_equation import ht_radiation_equation
from module.Sink.characterization.Building.Auxiliary.info_time_step_climate_data import info_time_step_climate_data

def greenhouse_changes(in_var):
    profile_minute_heat = []
    t_minute = []
    t_minute_real = []
    vector_1 = []
    ...
    L_ground = 3/2  # [m] - https://www.researchgate.net/publication/245189597_Spatial_variability_of_soil_temperature_under_greenhouse_conditions
    k_ground = 1.4  # [W/m.K]
    ...
    import pandas as pd
    greenhouse_t_wall = pd.read_csv('/module/Validation routines/teste_case_greenhouse.csv', sep=';')
    ...
    profile = [1] * 71
    total_cover_area_ratio =  1 - area_N_wall/(area_W_wall + area_N_wall + area_S_wall + area_E_wall + area_floor)
    ...
    for profile_index, profile_operating in enumerate(profile):
        ...
        T_exterior, T_sky, Q_sun_N_facade, Q_sun_S_facade, Q_sun_E_facade, Q_sun_W_facade, hour_sun_roof, wind_speed = info_time_step_climate_data(
            df_climate, profile_index, one_hour, 0)

        if profile_index + 1 != len(profile):
            T_exterior, T_sky, Q_sun_N_facade, Q_sun_S_facade, Q_sun_E_facade, Q_sun_W_facade, hour_sun_roof_final, wind_speed = info_time_step_climate_data(
                df_climate, profile_index+1, one_hour, 0)
        else:
            hour_sun_roof_final = 5

        print(profile_index,hour_sun_roof, hour_sun_roof_final)
        ...
        for i in range(one_hour):
            ...
            if hour_sun_roof == 0 or hour_sun_roof_final == 0:
                Q_sun_roof = 0
            ...
            Q_sun_greenhouse = (Q_sun_roof * area_floor) * tau_cover_solar_radiation

            #print('##############')
            #print((Q_sun_N_facade * area_N_wall + Q_sun_S_facade * area_S_wall + Q_sun_E_facade * area_E_wall + Q_sun_W_facade * area_W_wall + Q_sun_roof * area_floor))
            #print(Q_sun_roof * area_floor)
            ...
            if Q_sun_roof == 0:
                area_infiltration = total_cover_area * f_c * 0.75
            else:
            ...
            # Ground Losses
            Q_lost_soil = u_ground * area_floor * (T_soil - T_interior)
            ...
            if Q_sun_greenhouse == 0:
                ...
                Q_rad_ground = ht_radiation_equation(emissivity_cover_long_wave_radiation, total_cover_area*total_cover_area_ratio, T_interior, T_cover, view_factor)
            ...
            T_north_wall = interpolation(greenhouse_t_wall.loc[profile_index + 1, 't_wall'],
                          greenhouse_t_wall.loc[profile_index, 't_wall'], one_hour, 0, i,
                          greenhouse_t_wall.loc[profile_index, 't_wall'])

            h_vertical_north_wall = h_convection_vertical(T_north_wall, T_interior)
            Q_lost_exterior += area_N_wall * h_vertical_north_wall * (T_north_wall - T_interior)*0
            ...
            rho_ground = 1975
            cp_ground = 1480
            volume_ground = area_floor * 0.5
            Q_lost_ground = u_ground * area_floor * (T_ground - T_soil)

            T_soil = T_soil + (-Q_lost_soil + Q_lost_ground) * time_step / (rho_ground * cp_ground * volume_ground)
            ...
            if Q_heat_required > 0:
                cumulative_heat_monthly += Q_heat_required * time_step / 1000  # [kJ]
                cumulative_heat_hourly += Q_heat_required * time_step / 1000
            else:
                cumulative_cool_monthly += 0
                cumulative_cool_hourly += 0


            profile_minute_heat.append(Q_heat_required * time_step / 1000)

            vector_1.append(T_interior)
            vector_2.append(T_soil)

        # Hourly Profiles
        profile_hourly_heat.append(cumulative_heat_hourly)  # space heating demand [kWh]
        profile_hourly_cool.append(T_interior)  # space cooling demand [kWh]



    plt.plot(range(len(vector_1)),vector_1,label='T_interior')
    plt.plot(range(len(vector_2)),vector_2,label='T_soil')
    plt.title('TEMPERAAAAA')
    plt.legend()
    plt.show()

  #  plt.plot(range(len(t_minute_real)),t_minute_real,label='real')
    #plt.plot(range(len(t_minute)),t_minute,label='theo')
    #plt.legend()
   # plt.show()
