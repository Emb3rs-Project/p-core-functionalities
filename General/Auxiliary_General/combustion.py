...
def compute_flue_gas_temperature(kb : KB, power_equipment, fuel_type, m_fuel, m_flue_gas):
    ...
    for i in range(len(T_it)):
        iteration = False
        cp_flue_gas = medium.cp(fluid_type, (T_it[i] + initial_temperature) / 2)  # [kJ/kg.K]

        while iteration == False:
            T_chamber_new = (lhv_fuel * m_fuel) / (m_flue_gas / 3600 * cp_flue_gas) + initial_temperature  # [K]
            cp_flue_gas_new = medium.cp(fluid_type, (T_chamber_new + initial_temperature) / 2)  # [kJ/kg.K]
    ...
    for i in range(len(T_it)):
        iteration = False
        cp_flue_gas = medium.cp(fluid_type, (T_it[i] + T_chamber_max_value) / 2)  # [kJ/kg K]

        while iteration == False:

            T_flue_gas_outlet = -power_equipment / (m_flue_gas / 3600 * cp_flue_gas) + T_chamber_max_value  # [K]
            cp_flue_gas_new = medium.cp(fluid_type, (T_flue_gas_outlet + T_chamber_max_value) / 2)  # [kJ/kg K]
