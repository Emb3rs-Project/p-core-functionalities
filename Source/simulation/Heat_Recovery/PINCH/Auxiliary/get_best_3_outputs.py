...
def get_best_3_outputs(all_df,df_optimization):
    ...
    for index, row in df_optimization.iterrows():
        ...
        output_vector.append({
            ...
            'equipment_detailed_savings':all_df[int(df_optimization['index'])][1].to_dict(orient='records'),
            'pinch_hx_data': all_df[int(df_optimization['index'])][0].to_dict(orient='records')
            })
