...
def make_pinch_design_draw(streams_info,pinch_temperature,pinch_data,pinch_delta_temperature):
    ...
    ## get streams info - look on stream at a time a check above/below pinch
...
#
    ...
    for stream in streams_info:
        ...
        if supply_temperature > target_temperature:
            ...
            plt.gca().text(left_temperature + 0.5 , y_stream+height_temperature_text, "{:.1f}".format(supply_temperature) + 'º', ha="left", va="bottom")   # draw left temperature
            plt.gca().text(right_temperature - 2.5, y_stream+height_temperature_text, "{:.1f}".format(target_temperature)+ 'º', ha="left", va="bottom")   # draw right temperature
            ...
            if len(stream['below_pinch']) > 1:
                ...
                for iterator in range(len(stream['below_pinch'])):
                    if stream['below_pinch'][iterator]['id'] != stream['id']:
                        ...
                        print("target_temperature",target_temperature,"pinch_temperature", pinch_temperature)
                        if target_temperature > pinch_temperature - pinch_delta_temperature:
                            split_closest_pinch_temperature = pinch
                        else:
                            split_closest_pinch_temperature = left_temperature
